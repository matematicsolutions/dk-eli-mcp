...
Pulls a tiny static ``dk-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/dk-runtime.json
...
Opt-out: set ``DK_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "DK_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/dk-eli-mcp/releases/latest/download/dk-runtime.json.gz",
)
_CACHE_NAME = "dk-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "dk-eli-mcp-runtime (+https://github.com/matematicsolutions/dk-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("DK_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "dk-eli"
