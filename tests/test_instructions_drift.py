...
from dk_eli_mcp.server import INSTRUCTIONS, ToolError, mcp

SRC = (Path(__file__).parent.parent / "src" / "dk_eli_mcp" / "server.py").read_text(
    encoding="utf-8"
)
...
def test_instructions_only_reference_registered_tools():
    ...
    referenced_tools = {r for r in referenced if r.startswith("dk_")}
